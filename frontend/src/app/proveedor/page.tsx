...
export default function ProveedorDashboard() {
  const { data: hospData } = useQuery({ queryKey: ["prov", "dash", "hosp"], queryFn: () => hospedajesService.getMine() });
  const { data: actData } = useQuery({ queryKey: ["prov", "dash", "act"], queryFn: () => actividadesService.getMine() });
  const { data: transData } = useQuery({ queryKey: ["prov", "dash", "trans"], queryFn: () => transfersService.getMine() });
  const { data: vehData } = useQuery({ queryKey: ["prov", "dash", "veh"], queryFn: () => vehiculosService.getMine() });
  const { data: resData } = useQuery({ queryKey: ["prov", "dash", "res"], queryFn: async () => { const { data } = await api.get('/reservas/mis-reservas?limit=100'); return data; } });
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Mi Panel" description="Resumen de tus servicios y reservas" />
