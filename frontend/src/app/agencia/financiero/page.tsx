...
export default function AgenciaFinancieroPage() {
  const { data: hospData } = useQuery({ queryKey: ["agencia", "fin", "hosp"], queryFn: () => hospedajesService.getMine() });
  const { data: actData } = useQuery({ queryKey: ["agencia", "fin", "act"], queryFn: () => actividadesService.getMine() });
  const { data: transData } = useQuery({ queryKey: ["agencia", "fin", "trans"], queryFn: () => transfersService.getMine() });
  const { data: vehData } = useQuery({ queryKey: ["agencia", "fin", "veh"], queryFn: () => vehiculosService.getMine() });
  const { data: resData } = useQuery({ queryKey: ["agencia", "fin", "res"], queryFn: async () => { const { data } = await api.get('/reservas/mis-reservas?limit=100'); return data; } });
